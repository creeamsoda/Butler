...
# 公開済みで未視聴のアニメのリストを取得してメッセージをslackに送信
nowDate = datetime.date.today()
availaableList = UpdateSchedule.GetAvailableList(nowDate, saveData.Data["animes"])
index = 1
remindText = ""
for available in availaableList:
    remindText += f"{index}. {available["name"]} 第{available["nextEpisode"]}話\n"
    index += 1
...
if remindText != "":
    try:
        ...
        """
        # リスト本体はスレッドとして投稿
        listPostResult = client.chat_postMessage(
            channel=channel_id,
